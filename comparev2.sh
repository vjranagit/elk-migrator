...
compare_saved_objects() {
    ...
    missing_in_cloud=$(comm -23 <(printf "%s\n" "${local_spaces[@]}") <(printf "%s\n" "${cloud_spaces[@]}"))
    extra_in_cloud=$(comm -23 <(printf "%s\n" "${cloud_spaces[@]}") <(printf "%s\n" "${local_spaces[@]}"))
    common_spaces=$(comm -12 <(printf "%s\n" "${cloud_spaces[@]}") <(printf "%s\n" "${local_spaces[@]}"))

    echo -e "\n${GREEN}[+] Spaces with saved objects in both:${NC}"
    [ -n "$common_spaces" ] && echo "$common_spaces" || echo "  (none)"
...
echo -e "${YELLOW}Comparing resources in latest export folder: $latest_folder${NC}"

for resource in "${resources[@]}"; do
    if [[ -f "$CLOUD_ES/$resource" && -f "$LOCAL_ES/$resource" ]]; then
        compare_resources "$resource"
    else
        echo -e "\n${RED}❌ Missing $resource in one or both directories${NC}"
    fi
done

# Compare Kibana spaces
if [[ -f "$CLOUD_KB/kibana_spaces.json" && -f "$LOCAL_KB/kibana_spaces.json" ]]; then
    compare_spaces
else
    echo -e "\n${RED}❌ Missing kibana_spaces.json in one or both directories${NC}"
fi

# Compare Kibana saved objects per space
if [[ -d "$CLOUD_SAVED" && -d "$LOCAL_SAVED" ]]; then
    compare_saved_objects
else
    echo -e "\n${RED}❌ Missing saved objects directories in one or both environments${NC}"
fi
